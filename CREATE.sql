...
CREATE TABLE Adresy (
	ID int IDENTITY(1,1) PRIMARY KEY,
	adres_cz1 varchar(40) not null CHECK(adres_cz1 NOT LIKE '%[^A-Z/.ĄŻŹĘĆŃÓŁ 0-9]%'),
	adres_cz2 varchar(40) CHECK(adres_cz2 NOT LIKE '%[^A-Z/.ĄŻŹĘĆŃÓŁ 0-9]%'),
	kod_pocztowy char(6) not null CHECK (kod_pocztowy LIKE '[0-9][0-9]-[0-9][0-9][0-9]'),
	miejscowosc varchar(30) CHECK(miejscowosc NOT LIKE '%[^A-Z/.ĄŻŹĘĆŃÓŁ 0-9]%'),
	kraj varchar(58) CHECK(kraj NOT LIKE '%[^A-ZĄŻŹĘĆŃÓŁ 0-9]%'),
...
-- ZWIĄZKI + KASKAWOWA AKTUALIZACJA + KASKADOWE USUWANIE
